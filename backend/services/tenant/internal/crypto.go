...
// generateInviteToken returns a URL-safe random invitation token.
func generateInviteToken() (string, error) {
	b := make([]byte, 32)
	if _, err := rand.Read(b); err != nil {
		return "", fmt.Errorf("tenant: generate invite token: %w", err)
	}
	return base64.RawURLEncoding.EncodeToString(b), nil
}

// hashToken returns the hex-encoded SHA-256 of a token. Invitation tokens are
// stored and looked up by this hash so the plaintext never touches the database.
