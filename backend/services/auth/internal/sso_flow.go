...
// organization membership, issues the normal auth token pair, then stores that
// pair encrypted behind a short-lived one-time browser exchange code.
func (s *Service) CompleteSSOLogin(
	ctx context.Context,
	orgID, samlResponse, relayState string,
	meta RequestMeta,
) (*url.URL, error) {
	if s.ssoMembers == nil || s.ssoHandoffs == nil || s.ssoExchangeEncryptor == nil {
		return nil, apperrors.Internal("SSO is not configured")
	}
	...
	encryptedPayload, err := s.ssoExchangeEncryptor.Encrypt(payload)
	if err != nil {
		return nil, fmt.Errorf("encrypt SSO token handoff: %w", err)
	}
...
// ExchangeSSOCode atomically consumes a one-time code and returns the token
// pair produced by CompleteSSOLogin.
func (s *Service) ExchangeSSOCode(ctx context.Context, req SSOExchangeRequest) (*TokenPair, error) {
	if s.ssoHandoffs == nil || s.ssoExchangeEncryptor == nil {
		return nil, apperrors.Internal("SSO is not configured")
	}
	...
	payload, err := s.ssoExchangeEncryptor.Decrypt(exchange.EncryptedPayload)
	if err != nil {
